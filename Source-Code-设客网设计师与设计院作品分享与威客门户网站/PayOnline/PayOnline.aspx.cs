...
namespace ZoomLa.WebSite.Shop
{
    public partial class PayOnline : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            M_UserInfo info = buser.GetLogin(false);
            ...
            //订单状态放在提交中检测
            //检测End;
            payPlat = payPlatBll.GetPayPlatByid(pinfo.PayPlatID);
            if (!IsPostBack)
            {
                ...
                decimal vmoney = pinfo.MoneyReal;//支付金额 
                ...
                if (string.IsNullOrEmpty(PayMethod))
                {
                    ...
                    switch ((M_PayPlat.Plat)payPlat.PayClass)//现仅开通 12:支付宝即时到账和支付宝网银服务,15支付宝网银服务(上方代码中处理),银币与余额服务
                    {
                        ...
                        case M_PayPlat.Plat.UnionPay:
                            ...
                            v_url = urlReq1 + "/PayReceive.aspx?PayNo=" + PayNo;
                            ...
                            string wkey = MD5Key;				 // 如果您还没有设置MD5密钥请登录我们为您提供商户后台，地址：https://merchant3.chinabank.com.cn/
                        ...
                        case M_PayPlat.Plat.Alipay_Instant:
                            ...
                            con1.Return_url = urlReq1 + "/Return/AlipayReturn.aspx";
                            //付完款后服务器AJAX通知的页面 要用 http://格式的完整路径，不允许加?id=123这类自定义参数
                            con1.Notify_url = urlReq1 + "/Return/AlipayNotify.aspx";
                        ...
                        case  M_PayPlat.Plat.Alipay_Bank://支付宝网银,已上方处理
                            break;
                        case M_PayPlat.Plat.Mobo://成都MO宝支付
                            {
                            ...
                            break;
                        case M_PayPlat.Plat.WXPay://微信支付
                            Confirm_VMoney_Btn.Visible = true;
                            break;
                        case M_PayPlat.Plat.BaoFo:
                            #region 宝付
                            {
                                ...
                                dics.Add("Username", info.HoneyName);
                        ...
                        case M_PayPlat.Plat.Ebatong:
                            {
                                ...
                                ebatong_dics.Add("anti_phishing_key", new BLL.Ebatong.AskForTimestamp().askFor(payPlat.AccountID, payPlat.MD5Key));
                        ...
                        case M_PayPlat.Plat.ICBC_NC://南昌工行
                            {
                                ...
                                //Dictionary<string, string> dics = new Dictionary<string, string>();
                                //string data = icbc.SpliceTranData(pinfo, payPlat.AccountID, payPlat.SellerEmail, Request.Url.Host, siteurl + "ICBCNotify.aspx");
                                ...
                                //dics.Add("merCert", icbc.ReadCertToBase64(payPlat.PublicKey));
                                //LblHiddenValue.InnerHtml = new Pay_BaoFa().BuildForm("https://myipad.dccnet.com.cn/servlet/NewB2cMerPayReqServlet", dics);
                                //Alipay_Btn.Visible = true;
                            }
                            break;
                        case M_PayPlat.Plat.CashOnDelivery://货到付款
                            gateway = "/";
                ...
                else//非现金支付,给用户显示确认支付页,实际支付行为在Confirm_Click
                {
                    ...
                    //Paymoney.Visible = true;
                    Paymoney2.Visible = false;
                    LblOid.Text = pinfo.PaymentNum;
                    Plat.Text = "支付方式：";
                    M_OrderList mol = new M_OrderList();
                    ordertable = orderBll.GetOrderbyOrderNo(pinfo.PaymentNum);
                    if (ordertable != null && ordertable.Rows.Count > 0)
                    {
                        switch (PayMethod)
                        {
                            case "Purse":
                                ...
                                Fee.Text = "帐户剩余金额:";
                                LblRate.Text = info.Purse + "元";
                                LblPayPlatform.Text = "帐户余额支付";
                                break;
                            case "SilverCoin":
                                Titles.Text = "银币支付操作（确认支付款项)";
                                Fee.Text = "帐户剩余银币：";
                                LblRate.Text = info.SilverCoin + "元";
                                LblPayPlatform.Text = "账户银币支付";
                                break;
                            case "Score":
                                Titles.Text = "积分支付操作（确认支付款项)";
                                Fee.Text = "帐户剩余积分：";
                                LblRate.Text = info.UserExp + "分";
                                LblPayPlatform.Text = "账户积分支付";
                                break;
                            default:
                                break;
                        }
        ...
        protected void Confirm_Click(object sender, EventArgs e)
        {
            ...
            //{
            //    if (!StringHelper.MD5(Second.Text.Trim()).Equals(mu.PayPassWord))
            //    {
            //        function.WriteErrMsg("二级密码错误,请重新输入");
            //    }
            //}
            ...
            else if (!string.IsNullOrEmpty(PayMethod))//虚拟币支付
            {
                ...
                //        PayByVirtualMoney(PayMethod, orderMod);
                //    }
                //}
            }
        ...
        private void PayByVirtualMoney(string payMethod, M_Payment payMod)
        {
            ...
            switch (payMethod)//完成支付
            {
                case "Purse":
                    ...
                    mui.Purse = mui.Purse - (double)payMod.MoneyReal;
                ...
                case "SilverCoin":
                    ...
                    mui.SilverCoin = mui.SilverCoin - (double)payMod.MoneyReal;
                    buser.UpDateUser(mui);
                    zfpt.Text = "银币";
                    fees.Text = "剩余银币：";
                ...
                case "Score":
                    ...
                    mui.UserExp = (int)(mui.UserExp - (double)payMod.MoneyReal);
                    buser.UpDateUser(mui);
                    zfpt.Text = "积分";
                    fees.Text = "剩余积分：";
            ...
            for (int i = 0; i < orderList.Count; i++)//更改订单状态
            {
                ...
                {
                    case "Purse":
                        //mui.Purse = mui.Purse - orderMod.Ordersamount;
                        ...
                        {
                            M_UserExpDomP hist = new M_UserExpDomP() { UserID = mui.UserID, Operator = orderMod.Userid, OperatorIP = EnviorHelper.GetUserIP() };
                            hist.Detail = "会员" + orderMod.Rename + "的商城订单" + orderMod.OrderNo + "[购买生效]，扣除余额-" + Convert.ToDecimal(orderMod.Ordersamount).ToString("F2");
                            hist.Score = 0 - Convert.ToDouble(Convert.ToDecimal(orderMod.Ordersamount).ToString("F2"));
                    ...
                    case "SilverCoin":
                        ...
                        if (orderBll.Update(orderMod))
                        {
                            M_UserCoinHis hist = new M_UserCoinHis() { UserID = mui.UserID, Operator = orderMod.Userid, OperatorIP = EnviorHelper.GetUserIP() };
                            hist.detail = "会员" + orderMod.Rename + "的商城订单" + orderMod.OrderNo + "[购买生效]，扣除银币-" + Convert.ToDecimal(orderMod.Ordersamount).ToString("F2");
                            hist.SilverCoin = 0 - Convert.ToDouble(Convert.ToDecimal(orderMod.Ordersamount).ToString("F2"));
                    ...
                    case "Score":
                        ...
                        if (orderBll.Update(orderMod))
                        {
                            ...
                            {
                                UserID = mui.UserID,
                                Operator = orderMod.Userid,
                                detail = "会员" + orderMod.Rename + "的商城订单" + orderMod.OrderNo + "[购买生效]，扣除积分-" + Convert.ToDecimal(orderMod.Ordersamount).ToString("F2"),
                                score = (int)(0 - Convert.ToDouble(Convert.ToDecimal(orderMod.Ordersamount).ToString("F2")))
                            };
        ...
        private void PayByMoney(M_Payment payMod)
        {
            ...
            {
                case M_PayPlat.Plat.WXPay://微信支付,跳转至扫码页
                    {
                        ...
                        wxdata.SetValue("product_id", "1212122121");
        ...
        //虚拟币支付后,调用FinalStep后,根据需求做页面跳转,或显示数据
        ...
        public double GetTotal(DataTable dt)
        {
            double allAmount = 0;
            allAmount = (double)paymentBll.SelModelByPayNo(PayNo).MoneyReal;
            return allAmount;
        }
        #region 功能支持方法
