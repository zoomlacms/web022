...
public partial class Search_SearchBody : System.Web.UI.Page
{
    ...
    public DataTable SelPage(out int itemCount)
    {
        ...
        SqlParameter[] sp = new SqlParameter[] { new SqlParameter("key", "%" + KeyWord + "%") };
        return PageHelper.SelPage(PageSize, CPage, out itemCount, "ID", "*", "ZL_SearchView", where+" And (NodeID in (1,2) or NodeID in (select NodeID from ZL_Node Where ParentID in (1,2)))", order, sp);
    }
}