 //竞猜专用
if(!x_open_path)
    var x_open_path = '../../Images/guessGame/';	
...
function x_open(title, url, width, height){
	if (!x_open_ie5 && !x_open_ns6)
	    window.open(url, "", "width=width,height=height,scrollbars=no,resizable=no");
	else{
		x_open_win_id.style.display = '';
		initialwidth = width;
		initialheight = height ;
		change_size(initialwidth, initialheight);
		var clientRight = window.screen.availWidth - event.x - 400; 
		if(clientRight>= x_open_win_id.clientWidth) {
		    x_open_win_id.style.left =event.x+"px";
		    x_open_win_id.style.top = event.y + "px";
		}
		else
		{
		    x_open_win_id.style.left =(event.x-x_open_win_id.clientWidth)+"px";
		    x_open_win_id.style.top = event.y + "px";
		}
