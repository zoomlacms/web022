...
var litlp = "<li><div class='input-group'><span class='btn btn-default input-group-addon'>@op</span><textarea id='@op_t' style='width: 100%; height: 80px; ' value='@val'></textarea></div></li>";
//生成指定数目的选项
...
function AddOption(num) {
    ...
    for (var i = 0; i < num; i++) {
        ...
        $("#option_ul").append(li);
        UE.getEditor(opstr[i] + "_t", {
            toolbars: config.uebar
        });
    }
}
//解析选项信息
function AnalyOption(json) {
    ...
    for (var i = 0; i < json.length; i++) {
        ...
        $("#option_ul").append(li);
        var editor = UE.getEditor(json[i].op + "_t", {toolbars: config.uebar});
        editor.addListener("ready", function () {
            var val = json[$(this)[0].uid].val;
            $(this)[0].setContent(val);
        });
    }
}
//保存前执行
